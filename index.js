...
mongoose.connect(`mongodb+srv://sunny:sunnyh123@cluster0.x0yawgr.mongodb.net/NxtWatchProject?retryWrites=true&w=majority&appName=Cluster0`)
.then(()=>{
...
app.get("/savedvideos", async (req, res) => {
  ...
  app.get("/likedvideos", async (req, res) => {
    try {
      const likedVideos = await Videos.find({ likedStatus: "Liked" });
      return res.status(200).json({ likedVideos: likedVideos });
    } catch (error) {
      console.log(error);
    }
  });
